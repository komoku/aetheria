...
\subsubsection{Inventario (\textsf{Inventory})}
...
que es la que ``ve´ el inventario (\textsf{viewer}). Esto último es porque,
...
podría verse como ``El Códice de Antelys'' mientras que para un bárbaro
inculto el mismo libro podría ser ``un gran tomo''). Por lo tanto, siempre
...
de la siguiente forma: si el inventario está vacío, la cadena es ``nada.'',
mientras que si tiene cosas, es de la forma ``una espada, una moneda y un
...
pueden servir como bebida (usamos una propiedad ``bebida'' para marcar esos
...
definiríamos el comportamiento ``cuando el jugador va hacia el desierto'',
en lugar de ``cuando el jugador teclea ir norte en la habitación X'').
...
del inventario, todas las otras cosas se ``corren'' una posición hacia la
