...
\textsf{MultimediaInputOutputClient}:\footnote{el método \textsf{getIO()} de la
clase \textsf{Player} nos devuelve una instancia de
...
volver a crear los marcos con \textsf{addFrame()}.
...
Dicho método devuelve el panel que se utiliza internamente para representar el
frame, y puede ser utilizado por programadores que conozcan la API de Swing y
que quieran dibujar directamente sobre él. Por supuesto, no hace falta este
método para mostrar imágenes en frames, basta con los anteriores, que serán los
que interesen a la gran mayoría de usuarios.
