...
tenemos un plátano que puede ser comido. Cuando el jugador teclea \comillas{comer el
...
el jugador ha tecleado el verbo \comillas{comer} referido a esa entidad plátano, y
...
que actúen sobre una entidad dada, como \comillas{comer el plátano}, será más
...
comprobando cosas como si ha tecleado la palabra \comillas{plátano} como parte de
...
\item{Descripción: cualquier cosa que se nos ocurra (como por ejemplo \comillas{Un
bonito plátano.})}
...
\comillas{Código y propiedades} del plátano. Agrandamos el campo de código y, en el
menú contextual (botón derecho), seleccionamos \comillas{Insertar código -- Redefinir
...
un comando en el que mencione \comillas{plátano} (u otro nombre de referencia) y
...
ejemplo, si el jugador escribe \comillas{comer el plátano} en una habitación cuando
...
ejemplo \comillas{el plátano}.}
\end{itemize}

Así, por ejemplo, si queremos que cuando el jugador ponga \comillas{comer el plátano}
...
El motivo de que funcione tanto cuando ponemos \comillas{plátano} como \comillas{platano}
...
tanto \comillas{come} como \comillas{como} o \comillas{comer}, si en la condición del if hemos
...
Con el código que acabamos de ver, podemos proporcionar una \comillas{excusa} al
...
lleva, no dejar que se lo coma (diciendo algo así como \comillas{Primero tienes que
cogerlo}).}
...
\comillas{piel de plátano} creado de antemano pero no enlazado con el mundo, y que
cuando el jugador ponga el plátano demos el \comillas{cambiazo} quitando la entidad
plátano y sustituyéndola por la piel.

Así, creamos un objeto \comillas{piel de plátano} con las siguientes
características:
...
\subsection{Método de análisis de la entrada referida a dos entidades} \label{sec:entrada_dos_entidades}
...
\item{\comillas{Teletransportar} al jugador de una habitación a otra,}
...
\comillas{comer el plátano}, \comillas{encender la tele}, \comillas{pulsar el botón} o \comillas{frotar
la lámpara mágica}; pero no a \comillas{abrir el barril con la palanca}, \comillas{atar al
...
ello, usaremos la herramienta \comillas{Añadir personaje} (pero sin marcarlos como
...
así:
	...
	magia)}
	\item{Jugador: no}
	\item{Descripción: lo que queramos, por ejemplo \comillas{Un hombre triste y
...
contextual, y seleccionamos: \comillas{Insertar código -- Redefinir métodos de cosa --~
...
partes, una con cada entidad a la que se refiere: así, en \comillas{dar la moneda de
oro al mendigo}, \textsf{args1} será la cadena \comillas{la moneda de oro}, y
\textsf{args2} será \comillas{al mendigo}.
...
la que se ha referido el jugador. Así, si éste teclea \comillas{dar moneda al
mendigo}, \textsf{obj2} será la entidad mendigo (\verb|mobile("mendigo")|);
mientras que si teclea \comillas{dar moneda al embajador}, sería el objeto
...
manera: está claro que en este caso será más cómodo definir el comando \comillas{dar
...
caso), abrimos el menú contextual, y seleccionamos: \comillas{Insertar código --~
...
se puede decir \comillas{dar la moneda al mendigo}, pero también podría
...
menú contextual, seleccionamos \comillas{Insertar código -- Redefinir métodos de
...
diferencia de que si el jugador teclea las entidades \comillas{al revés} de como
las esperamos, el método les \comillas{da la vuelta} automáticamente.
...
Y este código funcionará exactamente igual para \comillas{dar la moneda al mendigo}
