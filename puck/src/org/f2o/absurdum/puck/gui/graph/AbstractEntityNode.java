/*
 * (c) 2005, 2007 Carlos Gómez Rodríguez, todos los derechos reservados / all rights reserved.
...
import org.f2o.absurdum.puck.gui.panels.AbstractEntityPanel;
 ...
 * @author carlos
 *
 * Created at regulus, 16-jul-2007 19:58:52
 */
public class AbstractEntityNode extends Node 
{
	
	private static int defaultSize = Integer.parseInt(PuckConfiguration.getInstance().getProperty("abstractEntityDisplaySize"));
	...
	public GraphElementPanel getAssociatedPanel()
	{
		...
		{
			associatedPanel = new AbstractEntityPanel( this );
		}
	...
	public Object clone()
	{
		return new AbstractEntityNode((int)bounds.getX(),(int)bounds.getY());
	}
	
	public AbstractEntityNode(int x, int y)
	{
	...
	static
	{
		try
		{
			...
			icon = ImageIO.read(AbstractEntityNode.class.getClassLoader().getResource(IMAGE_FILE));
		}
		catch ( Exception e ) 
		{
			;
	...
	*/
	...
	public void paint ( Graphics g , int x , int y , double zoom )
	{
		...
		*/
		...
		if ( PuckConfiguration.getInstance().getProperty("showAbstractEntityNodes").equals("true") || isHighlighted() )
		{
			g.drawImage(icon,x,y,(int)((int)bounds.getWidth()*zoom),(int)((int)bounds.getHeight()*zoom),null);
		}
		
		if ( PuckConfiguration.getInstance().getProperty("showAbstractEntityNames").equals("true") || isHighlighted() )
		{
