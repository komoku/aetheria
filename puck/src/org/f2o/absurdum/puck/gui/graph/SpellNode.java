...
public class SpellNode extends Node 
{
	...
	private static int defaultSize = Integer.parseInt(PuckConfiguration.getInstance().getProperty("spellDisplaySize"));
	...
	public GraphElementPanel getAssociatedPanel()
	{
		...
		{
			associatedPanel = new SpellPanel( this );
		}
	...
	public Object clone()
	{
		return new SpellNode((int)bounds.getX(),(int)bounds.getY());
	}
	
	public SpellNode(int x, int y)
	{
	...
	*/
	...
	public void paint ( Graphics g , int x , int y , double zoom )
	{
		...
		*/
		...
		if ( PuckConfiguration.getInstance().getProperty("showSpellNodes").equals("true") || isHighlighted() )
		{
	...
	public String getName()
	{
		...
		else return UIMessages.getInstance().getMessage("unnamed.spell");
	}
