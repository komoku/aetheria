...
//	procesa el comando decir
	...
	protected void escribirErrorNoEntiendo ( )
	{
		...
		write( io.getColorCode("denial") + DefaultMessages.getInstance().getMessage("error.parsing") + "\n" + io.getColorCode("reset") );
	...
	public boolean go ( Path p ) //throws java.io.IOException
	{
		...
		if ( !p.isValid() )
		{
			...
			//si no es un Informador, esto no hace nada.
				if ( p.isStandard() ) write( io.getColorCode("denial") + "No parece haber salida en esa dirección.\n" + io.getColorCode("reset") );
	...
	boolean getItem ( Item ourItem , Inventory inv , String toAppend )
	{
		...
		try
		{
				...
				if ( !ourItem.isGettable() )
				{
					...
					write( io.getColorCode("denial")  + DefaultMessages.getInstance().getMessage("denial.get.ungettable","$item",ourItem.constructName2True(1,this)) + "\n" + io.getColorCode("reset") );
					return true;
				}
	...
	protected boolean cogerItem ( String args , Inventory inv , String extraInfo )
	{
		...
		if ( patternMatchVectorSing.size() > 0 ) //cogemos un objeto
		{
			...
			try
			{
				...
				//Debug.println("Added: " + ourItem.constructName2(1,ourItem.getState()) );
				//decir que lo ha cogido
				escribir( io.getColorCode("action")  + lenguaje.gramaticalizar (  "Coges " + ourItem.constructName2True(1,this) + toAppend + "." ) + "\n" + io.getColorCode("reset") );
				...
				{
					escribir( io.getColorCode("error") + "EVASemanticException found at event_get , item number " + ourItem.getID() + io.getColorCode("reset") + "\n"  );
				}
				catch ( bsh.TargetError te )
				{
					escribir( io.getColorCode("error") + "bsh.TargetError found at event onGet , item number " + ourItem.getID() + io.getColorCode("reset") + "\n"  );
				}

				//describirlo
				escribir( io.getColorCode("description") + ourItem.getDescription(this) + '\n' + io.getColorCode("reset")  );
				...
				escribir( io.getColorCode("description") + pesoDescription + io.getColorCode("reset") + "\n" );
			...
			{
				escribir( io.getColorCode("denial")  + "¡Llevas demasiado peso para coger "+ ourItem.constructName2True(1,this) + "!\n" + io.getColorCode("reset") );
			}
			catch ( VolumeLimitExceededException ve )
			{
				escribir( io.getColorCode("denial")  + "¡Llevas objetos demasiado voluminosos para coger "+ ourItem.constructName2True(1,this) + "!\n" + io.getColorCode("reset") );
			}
		...
		else if ( patternMatchVectorPlur.size() > 0 )
		{
			...
			for ( int i = 0 ; i < patternMatchVectorPlur.size() ; i++ )
			{
				...
				try
				{
					...
					int pesoApr;
					pesoApr = (int)Math.round( factorApr*ourItem.getWeight() );
					escribir( io.getColorCode("description") + "Pesará unos " + pesoApr + " kilos.\n" + io.getColorCode("reset") );
				...
				catch ( WeightLimitExceededException we )
				{
					if ( i==0 )
						...
						escribir( io.getColorCode("denial")  + "¡Llevas demasiado pesos para coger más!\n" + io.getColorCode("reset") );
					break;
				}
				catch ( VolumeLimitExceededException ve )
				{
					if ( i==0 )
						...
						escribir( io.getColorCode("denial")  + "¡Llevas objetos demasiado voluminosos para coger más!\n" + io.getColorCode("reset") );
					break;	
				}
				*/
