...
entidades (como un valor de \comillas{simpatía} que mida cómo de simpático le cae
...
una entidad (aunque para relacionar entre sí dos entidades, como en \comillas{el
...
podríamos tener una propiedad \comillas{canal} a la que asignáramos un valor de
...
\subsection {Temporización y método \textsf{update}} \label{sec:Temporizacion}
...
consumen un número de unidades de tiempo que depende de la \comillas{longitud del
...
(\comillas{turnos} aparentes) y el modo de tiempo real. En el modo síncrono, cada
...
impresión de que se juega \comillas{por turnos}; pero no es exactamente así: por
...
la propiedad se actualizará. \comillas{Actualizarse} consiste en llamar a un método
...
El método \textsf{update} se llamará cada vez que el contador de una propiedad
...
(donde hemos puesto a la entidad del jugador el nombre único \comillas{jugador}),
...
$-1$ significa \comillas{infinito}, es decir, la propiedad tardará infinito en
actualizarse (no se actualizará nunca). Nótese también que \textbf{un temporizador
debería ponerse siempre a un valor positivo o bien a -1 (infinito), nunca a
cero. Poner un temporizador a cero no garantiza que el correspondiente método
update vaya a ejecutarse inmediatamente; sino que su comportamiento está
indefinido, así que debe evitarse}. Para ejecutar algún código inmediatamente,
es mejor simplemente llamar a ese código en lugar de usar temporizadores.
...
nos dará un error si antes no hemos fijado el valor de \comillas{encendido}, porque
...
panel de formularios de esa entidad y seleccionar la pestaña \comillas{Código y
...
botón \comillas{Añadir} y veremos en la lista cómo el valor inicial de nuestra
...
darle al botón \comillas{cambiar} para guardar los cambios. El botón \comillas{borrar} nos
...
televisor comience apagado. Poniendo el valor \comillas{true}, comenzaría encendido.
...
utilizamos una relación \comillas{conoce} de Fulanito a Menganito, que tomará valor
...
Juan siente por María. Para eso, utilizamos una relación \comillas{gusta} de Juan a
...
\comillas{gusta} del troll a cada uno de los objetos que le gusten, que tomará
...
PUCK, nos aparecerá un panel asociado a la flecha. En su ficha \comillas{Otras
relaciones}, nos aparecerá una lista de \comillas{Relaciones personalizadas} que
...
si son una habitación y una cosa se crea una relación \comillas{contiene} que
...
relaciones estructurales: esto se hace desmarcando el botón \comillas{hay camino},
entre habitaciones, o poniendo el campo \comillas{Relación estructural} a \comillas{Ninguna}
en la ficha \comillas{Relación estructural} del panel de la flecha, en el resto de
...
relación \comillas{gusta} del troll hacia esas cosas, sea a true o a false). Nótese
