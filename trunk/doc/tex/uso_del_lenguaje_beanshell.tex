...
tipado dinámico y tiene otras características donde ``relaja'' los requisitos
de Java.

Para añadir código BeanShell a un mundo con PUCK, se puede ir a la pestaña
``Código y Propiedades'' de cualquier entidad (las entidades son los objetos
...
área de texto (con un botón ``Ampliar'' que la convierte en una ventana
...
usamos para que cosa esté definido en la entidad ``máquina de coser': de
...
derecho, seleccionando la opción ``Insertar código -- Método de análisis de
...
parámetro corresponde a la palabra ``saludar'' (sin comillas), entonces
escribimos una línea que dice ``Hola.'' en la salida asociada a la criatura
...
cabecera se genera automáticamente en la opción ``Insertar código -- Método
...
de la cadena de entrada. Por ejemplo, si el jugador teclea ``comer el plátano
de Canarias'', entonces el parámetro aCreature corresponderá a ese jugador,
el parámetro verb a la cadena ``comer'', y args a la cadena ``el plátano de
Canarias''.
...
AGE le dirá ``Hola''. La sintaxis \verb|aCreature.write("Hola.\n")|
...
de línea. Por lo tanto, este código escribirá dos líneas, una que dirá ``Esta
línea se escribe primero.'' y otra que dirá ``Y ésta se escribe después.''.
...
podemos dar el valor ``Hola'' o $3.5$, porque éstos no son números enteros.
...
habitación ``cocina''), y, opcionalmente, otro código que sólo se ejecuta si
...
Una estructura ``if--else'' se puede combinar con otras estructuras
``if--else'' para dar una estructura ``if--else if--else if...--else'', como
ésta:
...
``inventario'' (y cualquier otro) porque el \textsf{end()} se ejecutaría
...
\textsf{if}, entonces al usar el verbo ``comer'' se imprimiría nuestro
...
si y sólo si el parámetro \textsf{verb} tiene como valor ``comer''; mientras
que \verb|!equals(verb,"comer")| devuelve true sólo si el valor del
parámetro no es ``comer''.
...
nosotros le pedimos que confirme poniendo ``sí'' o ``no'', podemos querer
pedirle esa confirmación mientras no dé una respuesta clara. Es decir, si le
preguntamos ``¿De verdad que quieres entrar en la cueva?'' y contesta
``Cachifú'', queremos volver a preguntarle. Esto sería un bucle que se
repetiría mientras la respuesta dada no sea ``sí'' ni ``no''.}
...
verbo ``saludar'', el juego muestre la palabra ``Hola'' cinco veces:
...
\textsf{continue}, que permiten que nuestro código ``haga trampa'' en un
...
Que sería otra manera de imprimir ``Hola'' cinco veces; aunque menos clara
