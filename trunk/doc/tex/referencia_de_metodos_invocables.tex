...
ejemplo, ``chorizo''), el segundo lo prefija con el artículo determinado
correspondiente (``el chorizo'', ``la cuchara'') y el tercero usa en su
lugar el artículo indeterminado (``un chorizo'', ``una cuchara''). La
...
devolverá ``Excalibur'' a secas, si ése es su nombre para mostrar y está
marcado como propio, mientras que si no fuese un nombre propio devolvería
``el Excalibur'', si la cosa es masculina, o ``la Excalibur'' si es
femenina.
...
cadena ``tres monedas''. Si pasamos como parámetro 1, estos métodos se
