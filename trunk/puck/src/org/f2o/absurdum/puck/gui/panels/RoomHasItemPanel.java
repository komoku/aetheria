...
public class RoomHasItemPanel extends ArrowPanel 
{
	...
	public RoomHasItemPanel( Arrow theArrow )
	{
		...
		relTypes.clear();
		relTypes.add(Messages.getInstance().getMessage("structural.room.item"));
		relTypes.add(ArrowPanel.NO_STRUCTURAL_RELATIONSHIP);
		
		//default structural relationship type
		this.relationshipType = Messages.getInstance().getMessage("structural.room.item"); //default relationship is this one (containment)
	...
	public void linkWithGraph()
	{
		...
		srcPanel.add ( new JLabel(Messages.getInstance().getMessage("roomitem.src")) );
		...
		dstPanel.add ( new JLabel(Messages.getInstance().getMessage("roomitem.dst")) );
