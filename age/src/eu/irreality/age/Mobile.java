...
public class Mobile extends Entity implements Descriptible , SupportingCode , Nameable
{
	...
	public boolean go ( Path p ) //throws java.io.IOException
	{
		...
		if ( !p.isValid() )
		{
			...
			if ( p.isStandard() ) write( io.getColorCode("denial") + 
					...
					+ io.getColorCode("reset") );
			else write( io.getColorCode("denial") + "�C�mo? �Ir a d�nde?\n" + io.getColorCode("reset") );
	...
	public String estimateDamage ( int damAmt )
	{
		...
		if ( percent < 5 )
			result = "muy ligeros da�os";
		else if ( percent < 10 )
			result = "ligeros da�os";	
		else if ( percent < 18 )
			result = "alg�n da�o";	
		else if ( percent < 25 )
			result = "un da�o significativo";
		else if ( percent < 33 )
			result = "da�os moderados";
		else if ( percent < 40 )
			result = "bastante da�o";
		else if ( percent < 50 )
			result = "severos da�os";
		else if ( percent < 60 )
			result = "mucho da�o";
		else if ( percent < 70 )
			result = "gran da�o";
	...
	public String estimateStatus (  )
	{
		...
		if ( percent <= 0.1 )
			...
			result = "tiene alg�n rasgu�o";
		else if ( percent < 10 )
			result = "tiene ligeras heridas";	
		else if ( percent < 18 )
			result = "tiene heridas poco graves";	
		else if ( percent < 25 )
			result = "tiene heridas de cierta consideraci�n";
		else if ( percent < 33 )
			...
			result = "est� cr�ticamente herido";
		else if ( percent < 90 )
	...
	public boolean checkPutInside ( Item ourContainer )
	{
		...
		{
			write ( io.getColorCode("denial") + "No parece muy �til poner cosas en " + ourContainer.constructName2True(1,this) + "." + io.getColorCode("reset") + "\n" );
			return false;
		}
